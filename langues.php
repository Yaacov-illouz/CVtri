...
function extractLanguages($text) {
    ...
    $extractedLanguages = array();
    // Recherche des langues dans le texte
    $text = strtolower($text); // Convertir le texte en minuscules pour une correspondance insensible à la casse
    foreach ($languages as $language) {
        // Vérifie si le nom de la langue est présent dans le texte
        if (strpos($text, $language) !== false) {
            $extractedLanguages[] = $language;
        }
    }
    // Affichage des langues extraites
    if (!empty($extractedLanguages)) {
        echo "Langues extraites : ";
        echo implode(", ", $extractedLanguages);
    } else {
        echo "Aucune langue trouvée dans le texte.";
    }
}
